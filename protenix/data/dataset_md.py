...
class BaseSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.group_by_pdb_id_wo_conf = kwargs.get("group_by_pdb_id_wo_conf", True)  # for md dataset
    ...
    def read_indices_list(self, indices_fpath: Union[str, Path]) -> pd.DataFrame:
        ...
        if self.group_by_pdb_id_wo_conf:
            indices_list['pdb_id_wo_conf'] = indices_list['pdb_id'].str.split('_').str[0]
        ...
        assert self.group_by_pdb_id + self.group_by_pdb_id_wo_conf <= 1, "only one of group_by_pdb_id and group_by_pdb_id_wo_conf can be True"
        ...
        if self.group_by_pdb_id_wo_conf:
            indices_list = [
                df.reset_index() for _, df in indices_list.groupby(["chain_1_id", "chain_2_id", "pdb_id_wo_conf"], sort=True)
            ]
        ...
        if self.sort_by_n_token:
            ...
            elif self.group_by_pdb_id_wo_conf:
                indices_list = sorted(
                    indices_list,
                    key=lambda df: int(df["num_tokens"].iloc[0]),
                    reverse=True,
        ...
        if self.error_dir is not None:
            if self.group_by_pdb_id_wo_conf:
                ...
                for i in range(len(sample_indice)):
                    data = sample_indice.iloc[i].to_dict()
                    data["error"] = error_message

                    filename = f"{sample_indice.iloc[i].pdb_id}-{sample_indice.iloc[i].chain_1_id}-{sample_indice.iloc[i].chain_2_id}.json"
    ...
    def _get_bioassembly_data(
        ...
        sample_indice = self._get_sample_indice(idx=idx)
        if self.group_by_pdb_id_wo_conf:
            bioassembly_dict_list, bioassembly_dict_fpath_list = [], []
            for pdb_id in sample_indice.pdb_id:
                bioassembly_dict_fpath = os.path.join(
                    self.bioassembly_dict_dir, pdb_id + ".pkl.gz"
                )
                bioassembly_dict_list.append(DataPipeline.get_data_bioassembly(bioassembly_dict_fpath=bioassembly_dict_fpath))
                bioassembly_dict_fpath_list.append(bioassembly_dict_fpath)
            return sample_indice, bioassembly_dict_list, bioassembly_dict_fpath_list
        else:
    ...
    @staticmethod
    def _get_shuffle_mol_id(token_array: TokenArray, atom_array: AtomArray):
        """
        Get the shuffled mol_id for the token_array and atom_array.
        """
        ...
        return shuffled_mol_ids
    
    def _shuffle_by_mol_id(self, token_array: TokenArray, atom_array: AtomArray, shuffled_mol_ids: np.ndarray):
        """
        Shuffle the token_array and atom_array based on the mol_id.
        """
        centre_atom_indices = token_array.get_annotation("centre_atom_index")
        token_mol_id = atom_array[centre_atom_indices].mol_id
    ...
    def _shuffle_array_based_on_mol_id(token_array: TokenArray, atom_array: AtomArray):
        ...
        """
        shuffled_mol_ids = BaseSingleDataset._get_shuffle_mol_id(token_array, atom_array)
        token_array, atom_array = BaseSingleDataset._shuffle_by_mol_id(token_array, atom_array, shuffled_mol_ids)
        return token_array, atom_array

    @staticmethod
    def _get_shuffle_sym_id(atom_array: AtomArray):
        """
        Generate shuffle indices for chains of the same entity_id
        e.g.
        when entity_id = 0
            sym_id_int = [0, 1, 2] -> shuffle_indices = [2, 0, 1]
        when entity_id = 1
            sym_id_int = [0, 1, 2, 3] -> shuffle_indices = [3, 0, 1, 2]
        """
        shuffle_sys_ids = {}
        
        for entity_id in np.unique(atom_array.label_entity_id):
            mask = atom_array.label_entity_id == entity_id
            sym_ids = atom_array.sym_id_int[mask]
            sym_ids_unique = np.sort(np.unique(sym_ids))
            sym_ids_shuffled = sym_ids_unique.copy()
            np.random.shuffle(sym_ids_shuffled)  # shuffle in-place
            map_dict = dict(zip(sym_ids_unique, sym_ids_shuffled))
            shuffle_sys_ids[entity_id] = map_dict
            
        return shuffle_sys_ids

    @staticmethod
    def _shuffle_by_sym_id(atom_array: AtomArray, shuffle_indices: dict):
        """
        Apply shuffle indices to atom_array
        """
        for entity_id, map_dict in shuffle_indices.items():
            mask = atom_array.label_entity_id == entity_id
            atom_array.sym_id_int[mask] = np.vectorize(map_dict.get)(atom_array.sym_id_int[mask])
        return atom_array
    ...
    def _assign_random_sym_id(atom_array: AtomArray):
        ...
        """
        shuffle_indices = BaseSingleDataset._get_shuffle_sym_id(atom_array)
        return BaseSingleDataset._shuffle_by_sym_id(atom_array, shuffle_indices)
    
    def _get_repr_bioassembly_random_info(self, idx: int):
        if self.group_by_pdb_id_wo_conf:
            # We select the first bioassembly_dict from the list as the representative bioassembly_dict
            # and
            sample_indice, bioassembly_dict_list, bioassembly_dict_fpath_list = (
                self._get_bioassembly_data(idx=idx)
            )
            sample_indice = sample_indice.iloc[0]
            bioassembly_dict = bioassembly_dict_list[0]
            bioassembly_dict_fpath = bioassembly_dict_fpath_list[0]
        else:
        ...
        if self.shuffle_mols:
            shuffled_mol_ids = BaseSingleDataset._get_shuffle_mol_id(bioassembly_dict["token_array"], bioassembly_dict["atom_array"])
            bioassembly_dict["token_array"], bioassembly_dict["atom_array"] = (
                BaseSingleDataset._shuffle_by_mol_id(bioassembly_dict["token_array"], bioassembly_dict["atom_array"], shuffled_mol_ids)
            )
        else:
            shuffled_mol_ids = None
        if self.shuffle_sym_ids:
            shuffle_sys_ids = BaseSingleDataset._get_shuffle_sym_id(bioassembly_dict["atom_array"])
            bioassembly_dict["atom_array"] = BaseSingleDataset._shuffle_by_sym_id(bioassembly_dict["atom_array"], shuffle_sys_ids)
        else:
            shuffle_sys_ids = None
        ...
        return shuffled_mol_ids, shuffle_sys_ids, crop_method, selected_indices, reference_token_index
    ...
    def process_one(
        ...
        if self.group_by_pdb_id_wo_conf:
            ...
            sample_indice = pd.DataFrame([sample_indice])
            bioassembly_dict_list = [bioassembly_dict]
            bioassembly_dict_fpath_list = [bioassembly_dict_fpath]

        shuffled_mol_ids, shuffle_sys_ids, crop_method, selected_indices, reference_token_index = self._get_repr_bioassembly_random_info(idx=idx)

        feat_list = []
        label_list = []
        label_full_list = []
        cropped_atom_array_list = []
        cropped_token_array_list = []
        for i in range(len(bioassembly_dict_list)):
            bioassembly_dict = bioassembly_dict_list[i]
            bioassembly_dict_fpath = bioassembly_dict_fpath_list[i]
            sample_indice_series = sample_indice.iloc[i]
            ...
            ref_chain_indices = DataPipeline._map_ref_chain(
                one_sample=sample_indice_series, bioassembly_dict=bioassembly_dict
            )
            ...
            label_full_list.append(label_full)
            cropped_atom_array_list.append(cropped_atom_array)
            cropped_token_array_list.append(cropped_token_array)

        if self.group_by_pdb_id_wo_conf:
            new_feat = feat_list[0]
            new_coord = torch.cat([label_list[i]['coordinate'].unsqueeze(0) for i in range(len(label_list))], dim=0)
            new_coord_full = torch.cat([label_full_list[i]['coordinate'].unsqueeze(0) for i in range(len(label_full_list))], dim=0)
            new_label = {k:v if k != 'coordinate' else new_coord for k, v in label_list[0].items()}
            new_label_full = {k:v if k != 'coordinate' else new_coord_full for k, v in label_full_list[0].items()}
        else:
            new_feat = feat_list[0]
            new_label = label_list[0]
            new_label_full = label_full_list[0]

        # Basic info, e.g. dimension related items
        basic_info = {
            "pdb_id": (
                bioassembly_dict_list[0]["pdb_id"]
                if self.is_distillation is False
                else sample_indice.iloc[0]["pdb_id"]
            ),
            "N_asym": torch.tensor([len(torch.unique(new_feat["asym_id"]))]),
            "N_token": torch.tensor([new_feat["token_index"].shape[0]]),
            "N_atom": torch.tensor([new_feat["atom_to_token_idx"].shape[0]]),
            "N_msa": torch.tensor([new_feat["msa"].shape[0]]),
            "bioassembly_dict_fpath": bioassembly_dict_fpath_list[0],
            "N_msa_prot_pair": torch.tensor([new_feat["prot_pair_num_alignments"]]),
            "N_msa_prot_unpair": torch.tensor([new_feat["prot_unpair_num_alignments"]]),
            "N_msa_rna_pair": torch.tensor([new_feat["rna_pair_num_alignments"]]),
            "N_msa_rna_unpair": torch.tensor([new_feat["rna_unpair_num_alignments"]]),
        }

        for mol_type in ("protein", "ligand", "rna", "dna"):
            abbr = {"protein": "prot", "ligand": "lig"}
            abbr_type = abbr.get(mol_type, mol_type)
            mol_type_mask = new_feat[f"is_{mol_type}"].bool()
            n_atom = int(mol_type_mask.sum(dim=-1).item())
            n_token = len(torch.unique(new_feat["atom_to_token_idx"][mol_type_mask]))
        ...
        asymn_id_to_chain_id = {
            atom.asym_id_int: atom.chain_id for atom in cropped_atom_array_list[0]
        }
        chain_id_list = [
        ...
        data = {
            ...
            "label_full_dict": new_label_full,
            "basic": basic_info,
        }

        if return_atom_token_array:
            data["cropped_atom_array"] = cropped_atom_array_list[0]
            data["cropped_token_array"] = cropped_token_array_list[0]
        return data
    ...
    def _get_sample_indice(self, idx: int) -> pd.Series:
        ...
        if self.group_by_pdb_id:
            ...
            sample_indice = self.indices_list[idx]
        else:
    ...
    def _get_pdb_indice(self, idx: int) -> pd.core.series.Series:
        if self.group_by_pdb_id:
            ...
            pdb_indice = self.indices_list[idx]
        else:
...
def get_sample_weights(
    ...
    if sampler_type == "weighted":
        ...
        if type(indices_df) == list:
            indices_df = pd.concat([i.iloc[0:1] for i in indices_df], ignore_index=True)
...
def get_datasets(
    ...
    for train_name in data_config.train_sets:
        ...
        train_dataset = BaseSingleDataset(**dataset_param, group_by_pdb_id_wo_conf=True)
    ...
    for test_name in test_sets:
        ...
        test_dataset = BaseSingleDataset(**dataset_param, group_by_pdb_id_wo_conf=False)
